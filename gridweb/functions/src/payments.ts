...
export const gpayCharge = functions.https.onCall(async (data, context) => {
  ...
  const { eventId, selectedTiers, promoCode, paymentMethodId, attendees, donor } =
    data;
...
export const stripeWebhook = functions.https.onRequest(async (req, res) => {
  ...
  if (evt.type === "checkout.session.completed") {
    ...
    if (order.orderType === "movie" && Array.isArray(order.seatIds) && order.seatIds.length > 0) {
      try {
        ...
        console.error(`Failed to fulfil movie seats for order ${orderId}:`, e?.message || e);
      }
    }
    ...
    if (orderId) {
      ...
      if (orderSnap.exists && orderSnap.data()?.status === "pending") {
        ...
        if (o.orderType === "movie" && Array.isArray(o.seatIds) && o.seatIds.length > 0) {
          const seatsCol = db.collection("events").doc(o.eventId).collection("seats");
          ...
          await batch.commit().catch((e) => console.error("Seat release on expiry failed", e));
        }
      }
    }
  }
