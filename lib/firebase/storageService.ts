...
/**
 * [NEW & FIXED] Uploads an image to a secure S3 bucket and returns the S3 key.
  ...
  try {
    ...
    const uploadResponse = await fetch(uploadUrl, {
      ...
      headers: {
        ...
        // 'x-amz-acl': 'public-read', // <<< FIX: ADDED THIS HEADER TO MATCH BACKEND
      },
    });

    if (!uploadResponse.ok) {
      // Log the detailed error from S3
      const errorText = await uploadResponse.text();
      console.log('S3 upload failed with status:', uploadResponse.status);
      console.log('S3 error response:', errorText, uploadResponse);
      return null
    ...
    console.log('Error in S3 upload process:', error);
