...
const uploadImageAndGetDownloadURL = async (
  imageUri: string,
  userId: string
): Promise<string | null> => {
  try {
    console.log(
      `[uploadImageAndGetDownloadURL] Starting upload for user: ${userId}`
    );

    // 1. Fetch the image data from the local URI
    console.log(
      "[uploadImageAndGetDownloadURL] Fetching image from URI:",
      imageUri
    );
    ...
    console.log(
      "[uploadImageAndGetDownloadURL] Created storage reference.",
      storageRef
    );

    // 4. Upload the blob to the reference
    console.log(
      "[uploadImageAndGetDownloadURL] Uploading blob to Firebase Storage..."
    );
    const uploadResult = await uploadBytes(storageRef, blob);
    console.log(
      "[uploadImageAndGetDownloadURL] Upload successful.",
      uploadResult
    );
    ...
    console.log(
      "[uploadImageAndGetDownloadURL] Obtained download URL:",
      downloadURL
    );
 ...
 */
const uploadImageAndGetS3Key = async (
  imageUri: string,
  isProfilePic: boolean = false
): Promise<string | null> => {
  return _uploadToS3(imageUri, { isProfilePic });
};

/**
 * [NEW] Uploads an image for a specific event to a secure S3 bucket and returns the S3 key.
 */
const uploadEventPhotoAndGetS3Key = async (
  imageUri: string,
  eventId: string
): Promise<string | null> => {
...
const _uploadToS3 = async (
  imageUri: string,
  payload: { eventId?: string; isProfilePic?: boolean }
): Promise<string | null> => {
  console.log(
    `[_uploadToS3] Starting S3 upload process with payload:`,
    payload
  );
  ...
  try {
    ...
    const { uploadUrl, s3Key } = result.data as {
      uploadUrl: string;
      s3Key: string;
    };
    if (!uploadUrl || !s3Key) {
