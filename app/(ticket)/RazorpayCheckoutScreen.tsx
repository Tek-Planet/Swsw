...
const RazorpayCheckoutScreen = () => {
  ...
  const validatePromoCode = async () => {
    ...
    try {
      ...
      if (error?.message?.includes("index")) {
        Alert.alert(
          "Configuration Error",
          "Promo codes are not configured correctly. Please contact support."
        );
      } else {
        Alert.alert(
          "Error",
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const options = {
        description: event?.title || "Ticket Payment",
        image: event?.coverImageUrl || "",
        currency: currency,
        key: "rzp_live_SNbWnQNoo2EwVU", // Replace with your key_id SMbyXFqOCAXC6i
        amount: amount,
        name: "Grid",
  ...
  return (
    <ThemedView style={styles.container}>
      <TopNavBar title="Order Summary UDI" onBackPress={() => router.back()} />
