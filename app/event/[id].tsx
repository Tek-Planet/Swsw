...
const EventDetailScreen = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenToEvent(id, async (eventData) => {
      ...
      if (eventData) {
        // Fetch host profile
        if (eventData.hostId) {
          const hostProfile = (await getProfilesForUserIds([eventData.hostId])).get(eventData.hostId);
          if (hostProfile) {
            setHost({ name: hostProfile.displayName, photoUrl: hostProfile.photoUrl });
          }
        }

        // Fetch ticket holder profiles
        const profilesMap = await getProfilesForUserIds(eventData.attendeeIds);
        const holdersList: TicketHolder[] = Array.from(profilesMap.entries()).map(([profId, profile]) => ({
          id: profId,
  ...
  const eventDate = event.startTime.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' });
  const eventTime = event.startTime.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
